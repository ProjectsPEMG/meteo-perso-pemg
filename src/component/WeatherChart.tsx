      ...
      <div className="flex flex-wrap justify-between items-center mb-6 gap-2">
        ...
        <div className="flex flex-wrap bg-[#0D1B2A] p-1 rounded-xl border border-slate-700 text-xs font-medium">
          <button 
            ...
            className={`px-3 py-1.5 rounded-lg transition-colors ${activeTab === 'temp' ? 'bg-[#F97316] text-white' : 'text-slate-400 hover:text-slate-200'}`}
          >
            Température
          </button>
          <button 
            onClick={() => setActiveTab('rain')}
            className={`px-3 py-1.5 rounded-lg transition-colors ${activeTab === 'rain' ? 'bg-[#38BDF8] text-white' : 'text-slate-400 hover:text-slate-200'}`}
          >
            Pluie & Humidité
          </button>
          <button 
            onClick={() => setActiveTab('wind')}
            className={`px-3 py-1.5 rounded-lg transition-colors ${activeTab === 'wind' ? 'bg-[#A78BFA] text-white' : 'text-slate-400 hover:text-slate-200'}`}
          >
            Vent & Rafales
          </button>
          <button 
            onClick={() => setActiveTab('aq')}
            className={`px-3 py-1.5 rounded-lg transition-colors ${activeTab === 'aq' ? 'bg-[#34D399] text-white' : 'text-slate-400 hover:text-slate-200'}`}
          >
            Pollution & Pollen
          </button>
        </div>
